...
def main():
    ...
    A_imgs = process_files(source_file_list, source_path, args.img_size)
    if A_imgs:
        A_imgs = np.concatenate(A_imgs, axis=-1).transpose((2, 0, 1))
        np.save(os.path.join(args.output, "source_imgs.npy"), A_imgs)
        print(f"A_imgs saved to {os.path.join(args.output, 'source_imgs.npy')}")

    # Process and save target files as B_imgs
    B_imgs = process_files(target_file_list, target_path, args.img_size)
    if B_imgs:
        B_imgs = np.concatenate(B_imgs, axis=-1).transpose((2, 0, 1))
        np.save(os.path.join(args.output, "target_imgs.npy"), B_imgs)
        print(f"B_imgs saved to {os.path.join(args.output, 'target_imgs.npy')}")
